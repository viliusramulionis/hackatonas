...
budy_interest = Blueprint('buddies_interest', __name__)
budy_interest_model = Budy()
...
# VISI VARTOTOJAI
@budy_interest.route("/", methods=['GET'])
def get_all():
    try :
        ...
        return jsonify(budy_interest_model.get_rows()), 200
    except :
...
@budy_interest.route("/<int:id>", methods=['GET'])
def get_one(id):
    try :
        return jsonify(budy_interest_model.get_row(id)), 200
    except :
        return jsonify("Atsiprašome, tačiau nepavyko gauti duomenų, bandykite dar kartą."), 500

# NAUJO VARTOTOJO KURIMAS
@budy_interest.route("/", methods=['POST'])
def create():
    try :
        
        budy_interest_model.insert_row(request.form)
...
@budy_interest.route("/<int:id>", methods=['PUT'])
def update(id):
    try :
        budy_interest_model.update_row(id, request.form)
        return jsonify("Jė jė!!! Vartotojo duomenys sėkmingai atnaujinti."), 200
    except :
        return jsonify("Atsiprašome, tačiau nepavyko užregistruoti vartotojo, įvyko klaida."), 500

# VARTOTOJO IŠTRYNIMAS
@budy_interest.route("/<int:id>", methods=['DELETE'])
def delete(id):
    try :
        budy_interest_model.delete_row(id)
